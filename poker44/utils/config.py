...
def add_args(cls, parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help="Neuron name used for the local log/state path.",
    )
    parser.add_argument(
        "--neuron.device",
        type=str,
...
def config(cls) -> bt.Config:
    ...
    # bittensor >= 10 no longer folds raw ``--namespace.arg`` options (added via
    # plain ``parser.add_argument``) into ``conf.<namespace>``; only its own
    # ``add_args`` helpers register. Rebuild the missing namespaces (neuron,
    # blacklist, wandb, miner, ...) from the parsed argparse values so all
    # downstream ``config.<ns>.<attr>`` access keeps working across versions.
    try:
        parsed, _ = parser.parse_known_args()
    except SystemExit:
        parsed = None
    if parsed is not None:
        # parse_known_args reflects CLI values (and argparse defaults); treat it
        # as authoritative since bittensor 10 may otherwise leave namespaces at
        # library defaults.
        for key, val in vars(parsed).items():
            if "." not in key:
                setattr(conf, key, val)
                continue
            ns_name, attr = key.split(".", 1)
            ns = conf.get(ns_name) if hasattr(conf, "get") else getattr(conf, ns_name, None)
            if ns is None:
                ns = bt.Config()
                setattr(conf, ns_name, ns)
            setattr(ns, attr, val)
    return conf
